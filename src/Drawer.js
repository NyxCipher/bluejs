...
export default class DrawerExample extends React.PureComponent<ExampleProps<BlueprintExampleData>, DrawerExampleState> {
     state: DrawerExampleState = {
        autoFocus: true,
     ...
     handleBackdropChange = handleBooleanChange(hasBackdrop => this.setState({ hasBackdrop }));
     ...
     handlePositionChange = handleValueChange((position: Position) => this.setState({ position }));

     handleOutsideClickChange = handleBooleanChange(val => this.setState({ canOutsideClickClose: val }));

     handleSizeChange = handleStringChange(size => this.setState({ size }));
